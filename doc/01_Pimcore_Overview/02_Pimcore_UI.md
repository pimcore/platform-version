...
The Editor Area is the main workspace. It displays whatever item you are currently editing — a product data object, an image asset, a website page, or any other element. The editor provides the relevant editing tools for the item type: form fields for data objects, a WYSIWYG editor for documents, image tools for assets.
...
## Hierarchical Trees and Folders

Data in Pimcore is organized hierarchically in **trees** — one tree each for Documents, Assets, and Data Objects. Each tree represents a folder structure where parent elements can contain child elements.

Folders serve a single purpose: helping users navigate and find items efficiently. They are not tied to any system logic — their structure is purely organizational and can be designed to match your team's workflows.

When you click on a folder:
- **Asset folders** show a thumbnail gallery of the contained files
- **Data Object folders** show a configurable list of the contained items, with support for bulk editing

The depth of the folder structure should serve usability — a flat structure with many items per folder can be as effective as a deep hierarchy, depending on the use case.


## Context Menus

Right-clicking any element or folder in a tree opens a **context-sensitive menu** with actions relevant to that specific item. For example, right-clicking an asset folder offers options like upload, new folder, rename, copy, and delete.
...
The Editor Console provides tabbed access to different aspects of the current item — for example View, Edit, Properties, Versions, Schedule, Dependencies, Notes & Events, Tags, and Workflow. Which tabs are available depends on the element type. Each tab corresponds to a shared capability that applies across all element types — see [Shared Capabilities](./03_Pimcore_Data_Elements.md#shared-capabilities) for what each one does.
...
A single user can have multiple perspectives and switch between them — useful when one role performs distinct tasks that require different tool arrangements. Combined with Pimcore's roles and permissions system, perspectives ensure that each user sees a clean, focused interface tailored to their responsibilities.
