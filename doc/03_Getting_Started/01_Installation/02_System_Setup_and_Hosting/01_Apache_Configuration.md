...
All rewrite rules required for Pimcore must be defined in the `.htaccess` file at `public/.htaccess`:
...
## Virtual Hosts
Make sure `AllowOverride All` is set for the `DocumentRoot` to enable `.htaccess` support.
...
<VirtualHost *:443>
        ...
        SSLEngine on
        SSLCertificateFile /etc/letsencrypt/live/YOURPROJECT.local/fullchain.pem
        SSLCertificateKeyFile /etc/letsencrypt/live/YOURPROJECT.local/privkey.pem
        ...
        ErrorLog ${APACHE_LOG_DIR}/YOURPROJECT.local_443_error.log
        CustomLog ${APACHE_LOG_DIR}/YOURPROJECT.local_443_access.log combined
</VirtualHost>
```
